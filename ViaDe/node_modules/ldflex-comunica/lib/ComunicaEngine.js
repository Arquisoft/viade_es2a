...
class ComunicaEngine {
  ...
  execute(sparql, source) {
    ...
    if (sparql.startsWith('INSERT') || sparql.startsWith('DELETE')) return this.executeUpdate(sparql, source); // Create an iterator function that reads the next binding
    ...
    function readNextBinding(resolve, reject) {
      if (errors.length > 0) return reject(errors.shift()); // Mark the iterator as done when the source has ended
  ...
  async toComunicaSources(source) {
    ...
    if (sources instanceof URL) sources = sources.href;else if (sources.termType === 'NamedNode') sources = sources.value;
    if (typeof sources === 'string') sources = [sources.replace(/#.*/, '')]; // Flatten recursive calls to this function
    else if (Array.isArray(sources)) sources = flatten((await Promise.all(sources.map(this.toComunicaSources)))); // Needs to be after the string check since those also have a match functions
      else if (sources.match) sources = [Object.assign({
          type: 'rdfjsSource'
