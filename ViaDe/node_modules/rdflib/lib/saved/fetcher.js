  ...
  (0, _createClass2.default)(Fetcher, [{
    ...
    value: function fetchUri(docuri, options) {
      ...
      return this._fetch(actualProxyURI, options).then(function (response) {
        ...
        return _this5.handleError(dummyResponse, docuri, options); // possible credentials retry @@@ gets lost!
    ...
    value: function getHeader(doc, header) {
      ...
      for (var r = 0; r < requests.length; r++) {
        ...
        if (request !== undefined) {
          var _response = kb.any(request, ns.link('response'));

          if (_response !== undefined && kb.anyValue(_response, ns.http('status')) && kb.anyValue(_response, ns.http('status')).startsWith('2')) {
            // Only look at success returns - not 401 error messagess etc
            var results = kb.each(_response, ns.httph(header.toLowerCase()));
    ...
    */
    ...
    value: function retryNoCredentials(docuri, options) {
      ...
      try {
        ...
        return this.failFetch(options, 'Failed ALSO without ccredentials: ' + err, response.status || 997, response);
      }
