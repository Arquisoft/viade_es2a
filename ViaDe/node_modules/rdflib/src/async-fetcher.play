  ...
  */
  ...
  async fetchUri (docuri, options) {
    ...
    if (Fetcher.unsupportedProtocol(docuri)) {
      return await this.failFetch(options, 'fetcher: Unsupported protocol', 'unsupported_protocol')
    }
    ...
    if (!options.force) {
      if (state === 'fetched') {  // URI already fetched and added to store
        return await this.doneFetch(options, {status: 200, ok: true, statusText: 'Already loaded into quadstore.'}
        )
      }
      if (state === 'failed' && this.requested[docuri] === 404) { // Remember nonexistence
        ...
        return await this.failFetch(options, message,
    ...
    try {
      let response = await this._fetch(actualProxyURI, options)
    } catch (error) {
      error => { // @@ handleError?
        let dummyResponse = {
          url: actualProxyURI,
    ...
    await this.handleResponse(response, docuri, options)
    return
  }
  }
