...
class Fetcher {
  ...
  */
   ...
   */
  async fetchUri (docuri, options) {
    if (!docuri) {
      throw new Error('Cannot fetch an empty uri')
    }

    if (Fetcher.unsupportedProtocol(docuri)) {
      return this.failFetch(options, 'fetcher: Unsupported protocol', 'unsupported_protocol') // throws
    }
    ...
    if (!options.force) {
      if (state === 'fetched') {
        return this.doneFetch(options, {status: 200, ok: true, statusText: 'Already loaded into quadstore.'})
      }
      if (state === 'failed' && this.requested[docuri] === 404) { // Remember nonexistence
        ...
        let response = await this.failFetch(options, message, this.requested[docuri], dummyResponse) // sorts out retries
        return response
      } else {
      // options.force == true
      ...
      try {
        var response = this._fetch(actualProxyURI, options)
      } catch (error) {
        let dummyResponse = {
          url: actualProxyURI,
        ...
        return this.handleError(dummyResponse, docuri, options) // possible credentials retry @@@ gets lost!
      }
      var resp = await this.handleResponse(response, docuri, options)
      return resp
    }
  }
 ...
 */
  ...
  retryNoCredentials (docuri, options) {
    ...
    try {
      ...
      console.log(`RETRY of ${docuri} WITHOUT CREDENTIALS FAILED too: <<<${err}>>>\n`)
        // return Promise.reject(@@)
      let response = err.response || {status: 996}
