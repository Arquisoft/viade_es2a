...
import { CommentWrapper, TextContainer } from "./comment.style";
...
const Comment = ({ route, comment }) => {
  ...
  const waypoint =
    comment.waypoint >= 0 && comment.waypoint < route.waypoints.length
      ? route.waypoints[comment.waypoint]
      : null;
  ...
  return (
    ...
    >
      ...
      <TextContainer>
        ...
        {waypoint && <span className="waypoint-name">{waypoint.name}</span>}
